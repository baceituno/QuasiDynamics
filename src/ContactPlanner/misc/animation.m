function animation(object, plan, nlp, vid_on)
	...
	drawArrow = @(x,y) quiver( x(1),y(1),x(2)-x(1),y(2)-y(1),0,'linewidth',3,'color','r')   
	drawArrow2 = @(x,y) quiver( x(1),y(1),x(2)-x(1),y(2)-y(1),0,'linewidth',3,'color','b')    
	drawArrow3 = @(x,y) quiver( x(1),y(1),x(2)-x(1),y(2)-y(1),0,'linewidth',3,'color','g')    
	...
	for t = 1:N_T
		...
		if no_shape == false
			t_x = t;
			for l = 1:nlp.N_l
				x = linspace(0,nlp.sol.value(nlp.vars.len(nlp.idx.len(1,l))),100);
				s = zeros(2,100);
				for o = 1:nlp.poly_order+1
					s_y = nlp.shape.coeff(nlp.idx.coeff(1,l,o))*basis_func(nlp.basis,x,o,1);
					s = s + [-sin(nlp.shape.trans(nlp.idx.trans(3,l,t_x)))*s_y;...
							 cos(nlp.shape.trans(nlp.idx.trans(3,l,t_x)))*s_y];
				end
				s_x = basis_func(nlp.basis,x,2,1);
				s = s + [cos(nlp.shape.trans(nlp.idx.trans(3,l,t_x)))*s_x;...
						 sin(nlp.shape.trans(nlp.idx.trans(3,l,t_x)))*s_x]; 
				plot(nlp.shape.trans(nlp.idx.trans(1,l,t_x))+s(1,:),nlp.shape.trans(nlp.idx.trans(2,l,t_x))+s(2,:),'LineWidth',2)
				xlim([-0.1,0.1])
				ylim([-0.1,0.2])
				hold on;
			end
