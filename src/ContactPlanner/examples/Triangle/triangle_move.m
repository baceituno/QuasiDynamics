classdef triangle_move
	...
	methods
		function obj = triangle_move()
			...
			th = linspace(0,-pi/3,5);
			y = (l/sqrt(3))*sin(pi/6 - th);
			x = (l/2)*(1 - cos(-th));

			th = [th, -pi/3*ones(1,5)];
			x = [x, x(5)*ones(1,5)];
			y = [y, linspace(y(5),y(5)+0.01,5)];
			...
			ext_f{2} = struct(); ext_f{2}.fc1 = [-1,4]; ext_f{2}.fc2 = [1,4];
