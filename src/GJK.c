...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
	...
	// counts
	size_t count1 = mxGetN(prhs[0]);
	size_t count2 = mxGetN(prhs[1]);

	// vertices
	vec2 vertices1[count1];
	vec2 vertices2[count2];
	...
	for(mwSize i = 0; i < count1; i++) {
		idx = i*2;
		vertices1[i].x = v1[idx];
		vertices1[i].y = v1[idx+1];
	}

	for(mwSize i = 0; i < count2; i++) {
		idx = i*2;
		vertices2[i].x = v2[idx];
		vertices2[i].y = v2[idx+1];
	}

	// double *outMatrix = mxGetPr(plhs[0]);
	// outMatrix[0] = gjk(vertices1,count1,vertices2,count2);
}